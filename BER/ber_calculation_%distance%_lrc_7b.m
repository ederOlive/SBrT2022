...
fid2 = fopen("~/Rx/Random_Vector_Sink_50B_%distance%_7b_%noise_level_1%.hex", "r");
...
fid2 = fopen("~/Rx/Random_Vector_Sink_50B_%distance%_7b_%noise_level_2%.hex", "r");
vector_rx_tooBig = fread(fid2, Inf, "uchar"); % reads received vector
vector_rx = vector_rx_tooBig(1:length(vector_tx)*floor(length(vector_rx_tooBig)/length(vector_tx)), 1);
fclose(fid2);
clear fid2 vector_rx_tooBig;
...
fid2 = fopen("~/Rx/Random_Vector_Sink_50B_%distance%_7b_%noise_level_n%.hex", "r");
vector_rx_tooBig = fread(fid2, Inf, "uchar"); % reads received vector
vector_rx = vector_rx_tooBig(1:length(vector_tx)*floor(length(vector_rx_tooBig)/length(vector_tx)), 1);
fclose(fid2);
clear fid2 vector_rx_tooBig;
...
lrc_7b_ber = [ber_mean_noise_level_1, ber_mean_noise_level_2, ..., ber_mean_noise_level_n];
csvwrite("lrc_7b_ber_%distance%.csv", lrc_7b_ber);
